...
@section('content')
<div class="container">
    <h2>Editar Disciplina: {{ $disciplina->nome }}</h2>
    ...
    <form action="{{ route('disciplinas.update', $disciplina->id) }}" method="POST">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" name="nome" id="nome" class="form-control @error('nome') is-invalid @enderror" value="{{ old('nome', $disciplina->nome) }}" required>
            @error('nome')
        ...
        <div class="form-group mb-3">
            ...
            <input type="number" name="carga_horaria" id="carga_horaria" class="form-control @error('carga_horaria') is-invalid @enderror" value="{{ old('carga_horaria', $disciplina->carga_horaria) }}" required min="1">
            @error('carga_horaria')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group mb-3">
            ...
            <select name="modulo" id="modulo" class="form-control @error('modulo') is-invalid @enderror" required>
                ...
                @foreach($modulos as $modulo)
                    <option value="{{ $modulo }}" {{ (old('modulo', $disciplina->modulo) == $modulo) ? 'selected' : '' }}>
                        Módulo {{ $modulo }}
                    </option>
                @endforeach
            </select>
