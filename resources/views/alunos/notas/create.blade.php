{{-- resources/views/alunos/notas/create.blade.php --}}
@extends('layouts.app')

@section('content')
<div class="container">
    <h2>Registrar Nova Nota para {{ $aluno->nome }}</h2>

    <form action="{{ route('alunos.notas.store', $aluno->id) }}" method="POST">
        @csrf
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('nota') is-invalid @enderror" id="nota" name="nota" step="0.01" min="0" max="10" value="{{ old('nota') }}" required>
            @error('nota')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
